...
DisXcon::DisXcon(settings_array& settings)
	:DisCell{ settings },
	m_currentConstantContaminantf{ settings[s_contaminantMobility] * settings[s_dt] / m_dxf },
	m_currentConstantContaminants1{ settings[s_contaminantMobility] * settings[s_dt] / m_dxs1 },
	m_currentConstantContaminants2{ settings[s_contaminantMobility] * settings[s_dt] / m_dxs2 }
{
...
void DisXcon::initializeConcentrations(double contaminantConcentration)
{
...
void DisXcon::calculatePotentialProfile()
{
	...
	for (array_type::size_type i{ 1 }; i < m_interfacePoint; ++i)
	{
		m_electrostatic[es_secondDerivative][i] = m_poissonConstantFilm * (-m_concentrations[carrier_electrons][i] + m_concentrations[carrier_cations][i]
			- m_concentrations[carrier_anions][i] - m_contaminations[contamination_Xmin][i]);
	}
	...
	while (std::abs(potAtReference) > 0.0001) //typically takes 5 rounds, quite a lot but for now its ok
	{
		...
		while (std::abs(differenceEF) > 0.00001) //typically takes 2 rounds, very good!
		{
			...
			for (array_type::size_type i{ 0 }; i < m_interfacePoint - 1; ++i) //build electric field sequentially using said boundary condition
			{
				m_electrostatic[es_electricField][i + 1] = m_electrostatic[es_electricField][i] - m_electrostatic[es_secondDerivative][i + 1] * m_dxf;
			}


			for (array_type::size_type i{ m_interfacePoint - 1 }; i < (m_interfacePoint + 10); ++i) //build electric field sequentially using said boundary condition
			{
				m_electrostatic[es_electricField][i + 1] = m_electrostatic[es_electricField][i] - m_electrostatic[es_secondDerivative][i + 1] * m_dxs1;
			}

			for (array_type::size_type i{ m_interfacePoint + 10 }; i < (m_size - 2); ++i) //build electric field sequentially using said boundary condition
			{
			...
			electricFieldIntegral = std::accumulate(m_electrostatic[es_electricField].begin(), m_electrostatic[es_electricField].begin() + m_interfacePoint - 1, 0.0) * m_dxf;
			electricFieldIntegral += std::accumulate(m_electrostatic[es_electricField].begin() + m_interfacePoint - 1, m_electrostatic[es_electricField].begin() + m_interfacePoint + 10, 0.0) * m_dxs1;
			electricFieldIntegral += std::accumulate(m_electrostatic[es_electricField].begin() + m_interfacePoint + 10, m_electrostatic[es_electricField].end(), 0.0) * m_dxs2;
		...
		for (array_type::size_type i{ 0 }; i < m_interfacePoint; ++i) // update potential profile using the aquired electrid field
		{
			m_electrostatic[es_potential][i + 1] = m_electrostatic[es_potential][i] - m_electrostatic[es_electricField][i] * m_dxf;
		}


		for (array_type::size_type i{ m_interfacePoint }; i < (m_interfacePoint + 11); ++i) // update potential profile using the aquired electrid field
		{
			m_electrostatic[es_potential][i + 1] = m_electrostatic[es_potential][i] - m_electrostatic[es_electricField][i] * m_dxs1;
		}

		for (array_type::size_type i{ m_interfacePoint + 11 }; i < (m_size - 1); ++i) // update potential profile using the aquired electrid field
		{
...
inline double DisXcon::negativeCurrent(const double concentrationLeft, const double concentrationRight, const double curCon, const double electricField, const double eCon)
{
...
inline double DisXcon::negativeCurrente(const double concentrationLeft, const double concentrationRight, const double curCon, const double electricField, const double eCon)
{
	//returns the amount of negatively charged particles moving to the right cell per m3 per timestep, based on drift/diffusion
	//energyConvertX (merged for speed) = k*T/q/dx
	//curCon (merged for speed)		= mobility*dt/dx
...
inline double DisXcon::positiveCurrent(const double concentrationLeft, const double concentrationRight, const double curCon, const double electricField, const double eCon)
{
...
inline double DisXcon::neutralCurrent(const double concentrationLeft, const double concentrationRight, const double curCon, const double eCon)
{
	//returns the amount of negatively charged particles moving to the right cell per m3 per timestep, based on drift/diffusion
	//energyConvertX (merged for speed) = k*T/q/dx
	//curCon (merged for speed)		= mobility*dt/dx
...
void DisXcon::react()
{
	...
	for (array_type::size_type i{ 1 }; i < m_interfacePoint+1; ++i)
	{
		...
		m_contaminationCurrents[contamination_Xswitch][i] = (m_contaminations[contamination_X][i] < m_concentrations[carrier_electrons][i]? m_contaminations[contamination_X][i] : m_concentrations[carrier_electrons][i]);
	}
	...
	*/
	...
	{
		//update all the concentrations based on the oxidation reaction
		m_contaminations[contamination_Xmin][i] += m_contaminationCurrents[contamination_Xswitch][i];
	}

	for (array_type::size_type i{ 1 }; i < m_interfacePoint+1; ++i)
	{
		//update all the concentrations based on the oxidation reaction
		m_contaminations[contamination_X][i] -= m_contaminationCurrents[contamination_Xswitch][i];
	}
}
void DisXcon::calculateCurrents()
{
	...
	for (array_type::size_type i{ m_interfacePoint + 2 }; i < (m_interfacePoint + 11); ++i)
	{
		m_currents[carrier_cations][i] = positiveCurrent(m_concentrations[carrier_cations][i - 1], m_concentrations[carrier_cations][i],
			m_currentConstantCationsSolution1, m_electrostatic[es_electricField][i - 1], m_energyConvertxs1);
	}

	for (array_type::size_type i{ m_interfacePoint + 11 }; i < (m_size - 1); ++i)
	{
		m_currents[carrier_cations][i] = positiveCurrent(m_concentrations[carrier_cations][i - 1], m_concentrations[carrier_cations][i],
	...
	m_currents[carrier_cations][m_interfacePoint + 1] = positiveCurrent(m_concentrations[carrier_cations][m_interfacePoint], m_concentrations[carrier_cations][m_interfacePoint + 1] * m_QDFillFactor,
		m_currentConstantCationsFilm * m_dxf / m_dxs1, m_electrostatic[es_electricField][m_interfacePoint], m_energyConvertxs1);
	m_currents[carrier_anions][m_interfacePoint + 1] = negativeCurrent(m_concentrations[carrier_anions][m_interfacePoint], m_concentrations[carrier_anions][m_interfacePoint + 1] * m_QDFillFactor,
		m_currentConstantAnionsFilm * m_dxf / m_dxs1, m_electrostatic[es_electricField][m_interfacePoint], m_energyConvertxs1);
	
	//Added these for X and Xmin
	for (array_type::size_type i{ 2 }; i < m_interfacePoint + 1; ++i)
	{
		m_contaminationCurrents[contamination_Xmin][i] = negativeCurrent(m_contaminations[contamination_Xmin][i - 1], m_contaminations[contamination_Xmin][i],
			m_currentConstantContaminantf, m_electrostatic[es_electricField][i - 1], m_energyConvertxf);
	}

	for (array_type::size_type i{ m_interfacePoint + 2 }; i < (m_interfacePoint + 11); ++i)
	{
		m_contaminationCurrents[contamination_Xmin][i] = negativeCurrent(m_contaminations[contamination_Xmin][i - 1], m_contaminations[contamination_Xmin][i],
			m_currentConstantContaminants1, m_electrostatic[es_electricField][i - 1], m_energyConvertxs1);
	}

	for (array_type::size_type i{ m_interfacePoint + 11 }; i < (m_size - 1); ++i)
	{
		m_contaminationCurrents[contamination_Xmin][i] = negativeCurrent(m_contaminations[contamination_Xmin][i - 1], m_contaminations[contamination_Xmin][i],
			m_currentConstantContaminants2, m_electrostatic[es_electricField][i - 1], m_energyConvertxs2);
	}
	...
	for (array_type::size_type i{ 2 }; i < m_interfacePoint + 1; ++i)
	{
		m_contaminationCurrents[contamination_X][i] = neutralCurrent(m_contaminations[contamination_X][i - 1], m_contaminations[contamination_X][i],
			m_currentConstantContaminantf, m_energyConvertxf);
	}

	for (array_type::size_type i{ m_interfacePoint + 2 }; i < (m_interfacePoint + 11); ++i)
	{
		m_contaminationCurrents[contamination_X][i] = neutralCurrent(m_contaminations[contamination_X][i - 1], m_contaminations[contamination_X][i],
			m_currentConstantContaminants1, m_energyConvertxs1);
	}
	...
	{
		m_contaminationCurrents[contamination_X][i] = neutralCurrent(m_contaminations[contamination_X][i - 1], m_contaminations[contamination_X][i],
			m_currentConstantContaminants2, m_energyConvertxs2);
	}

	m_contaminationCurrents[contamination_X][m_interfacePoint + 1] = neutralCurrent(m_contaminations[contamination_X][m_interfacePoint], m_contaminations[contamination_X][m_interfacePoint + 1],
		m_currentConstantContaminants1, m_energyConvertxs1);

	m_contaminationCurrents[contamination_Xmin][m_interfacePoint + 1] = negativeCurrent(m_contaminations[contamination_Xmin][m_interfacePoint], m_contaminations[contamination_Xmin][m_interfacePoint + 1],
		m_currentConstantContaminants1, m_electrostatic[es_electricField][m_interfacePoint], m_energyConvertxs1);

	m_currentCumulative -= m_currents[carrier_electrons][1];
}
void DisXcon::updateConcentrations()
{
	...
	m_concentrations[carrier_cations][m_interfacePoint] -= m_currents[carrier_cations][m_interfacePoint + 1] * m_dxs1 / m_dxf;
	m_concentrations[carrier_cations][m_interfacePoint + 11] -= m_currents[carrier_cations][m_interfacePoint + 12] * m_dxs2 / m_dxs1;
	...
	m_concentrations[carrier_anions][m_interfacePoint] -= m_currents[carrier_anions][m_interfacePoint + 1] * m_dxs1 / m_dxf;
	m_concentrations[carrier_anions][m_interfacePoint + 11] -= m_currents[carrier_anions][m_interfacePoint + 12] * m_dxs2 / m_dxs1;
	...
	m_contaminations[contamination_X][m_interfacePoint] -= m_contaminationCurrents[contamination_X][m_interfacePoint + 1] * m_dxs1 / m_dxf;
	m_contaminations[contamination_X][m_interfacePoint + 11] -= m_contaminationCurrents[contamination_X][m_interfacePoint + 12] * m_dxs2 / m_dxs1;


	//Xmin
	for (array_type::size_type i{ 1 }; i < (m_size - 1); ++i)
	{
		m_contaminations[contamination_Xmin][i] += m_contaminationCurrents[contamination_Xmin][i] - m_contaminationCurrents[contamination_Xmin][i + 1];
	}
	m_contaminations[contamination_Xmin][m_interfacePoint] -= m_contaminationCurrents[contamination_Xmin][m_interfacePoint + 1] * m_dxs1 / m_dxf;
	m_contaminations[contamination_Xmin][m_interfacePoint + 11] -= m_contaminationCurrents[contamination_Xmin][m_interfacePoint + 12] * m_dxs2 / m_dxs1;

	m_contaminations[contamination_X][m_referencePoint - 1] += m_contaminations[contamination_Xmin][m_referencePoint - 1];
	m_contaminations[contamination_Xmin][m_referencePoint - 1] = 0;
}
void DisXcon::midSave(std::ofstream& midf)
{
	{
		for (int i{ 0 }; i < 3; ++i)
		{
